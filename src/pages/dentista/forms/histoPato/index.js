...
const FormHistoPato = () => {
  const initialValues = Object.values(form.Form).reduce((acc, field) => {
    return { ...acc, [field.title]: field.textarea ? "" : [] };
  }, {});
  ...
  const { value, onChangeHandler, clearForm, onChangeHandlerText } =
    useFormOptions(initialValues);
      ...
      >
        ...
        {Object.entries(form.Form).map(([sectionKey, section]) => (
          <div key={sectionKey}>
            <TitleSectionForm>{section.titleSection}</TitleSectionForm>
              {Object.values(section.Questions).map((field) => (
                <div key={field.title}>
                      ...
                      <>
                        ...
                        {value[field.title]?.includes("Outro") && (
                          <TextInput
                            type="text"
                            name={field.title}
                            value={value[field.title + "_outro"] || ""}
                            onChange={onChangeHandlerText}
                          />
                        )}
                      </>                    
        ...
        >
          <DialogTitle id="alert-dialog-title">
            Selecione Paciente e Laboratório
          </DialogTitle>
          <div>
            <SelectBox
              options={Patients}
